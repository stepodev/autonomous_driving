...
roslaunch platooning_sim sim_2cars.lauch
	...
	\label{hw_architektur}
		\subsection{Hardware}
		\label{hardware}
		Dieser Abschnitt dient der Nachvollziehbarkeit wie der Umbau des Modellautos  „LOSI 8ight-E 4WD Buggy 1:8 RTR“ 
		durchgeführt wird. Sinn dieser Anleitung ist, dass mit diesem Dokument und den erforderlichen Teilen auch Dritte 
		den Umbau durchführen können. Dazu ist hier eine Teileliste und eine bebilderte Schritt-für-Schritt Anleitung enthalten.
		...
		Die bebilderte Anleitung zum Fahrzeugzusammenbau befindet sich im Anhang \ref{appendix:fahrzeugbau}. 
		\subsubsection{Teileliste Hardware}
		    \begin{itemize}
				\item LOSI 8ight-E 4WD Buggy 1:8 RTR (RC Auto)
				\item Aluminium-Platine für die Boards (Assystem)
				\item SRF02 Ultraschall Entfernungssensor
				\item STM32F4-Discovery Kit STM32F407G-DISC1
				\item ODROID-XU4 Einplatinen-Computer
				\item Platinenkamera, USB 2.0, 1/3" CMOS
				\item Mini-Objektiv 12x0.5mm
				\item Kamerahalterung (3D-Druck Assystem)
				\item Hintere Aluminium-Platinen Halterung (3D-Druck Assystem)
				\item Halterung Front-Stoßfänger (3D-Druck Assystem)
				\item Halterung Front-Ultraschallsensor (3D-Druck Assystem)
				\item Wheel-Encoder Achsaufnahme (3D-Druck Assystem)
				\item Halterung Wheel-Encoder Sensor (3D-Druck Assystem)
				\item 2x LiPo Akku
			\end{itemize}
		\subsubsection{Teileliste Kabel}
		Die folgende Teileliste beschreibt die benötigten Kabel. In Klammern steht jeweils ob auf der Seite des Anschlusses 
		ein x-poliger Stecker benötigt wird. Die Verbindungen zum Odroid ist jeweils mit 1-poligen Steckern pro Ader versehen. 
		Für ein besseres Verständnis ist jedes Kabel im Folgenden bebildert. Die in der späteren Beschreibung genannten Farben 
		der Adern kann von zukünftig gelieferten Farben abweichen. Deshalb immer sichergehen, dass die richtigen Kontakte 
		zwischen den Boards verbunden werden. Die Liste inklusive Bilder der Kabel befindet sich im Anhang.
		\subsubsection{Teileliste Schrauben}
		Die Liste der benötigten Schrauben und Abstandshalter inklusive Bilder befindet sich im Anhang.
	...
	\label{sw_architektur}
		...
		% Beschreibung der Nodes inkl. (Aktivitäts-)Diagramme
			...
			\label{platooning}
				...
				\paragraph{Methoden}
					\subparagraph{reset} Setzt alle laufenden Timer zurück.
					\subparagraph{hndl\_msg\_fv\_leave} Entfernt Fahrzeuge aus dem Platoon, welche eine Anfrage auf Verlassen des Platoons gestellt haben.
					\subparagraph{hndl\_msg\_lv\_accept} Startet die lv\_broadcast und fv\_hearbeat Timer.
					\subparagraph{hndl\_msg\_lv\_reject} Ist das Fahrzeug im Status FV, werden rejects ignoriert und der Heartbeat-Broadcast weiter gesendet.
					\subparagraph{hndl\_msg\_fv\_request} Abhängig ob das Fahrzeug LV ist und sich im State RUNNING oder CREATING befindet, wird ein anfragendes Fahrzeug hinzugefügt und die entsprechenden Timer gestartet.
					\subparagraph{hndl\_msg\_lv\_broadcast} Aktualisiert bei Empfang der Broadcast-Nachricht die Werte für Speed, Platoon Distance, Platoon Member.
					\subparagraph{hndl\_msg\_fv\_heartbeat} Bei Empfang des fv\_heartbeat wird der Timer für das FV aktualisiert.
					\subparagraph{hndl\_msg\_platooning\_toggle} Abhängig davon, ob das Platooning an sich aktiviert ist und ob das Fahrzeug FV ist, wird das Platooning deaktiviert. Andernfalls wird der CREATE-Modus gestartet. Ist das Fahrzeug LV wird gewartet, ist es FV wird ein FV\_REQUEST gesendet.
					\subparagraph{update\_state} Aktualisiert den internen Platoonstatus basierend auf der empfangenen Broadcastnachricht.
					\subparagraph{hndl\_timeout\_fv\_heartbeat} Überprüft ob jedes Platoonmitglied sich über Heartbeats meldet. Wenn nicht, wird dieses aus dem Platoon entfernt und der Timer zurück gesetzt.
					\subparagraph{hndl\_timeout\_lv\_broadcast} Überprüft, wann der letzte Broadcast stattgefunden hat. Wenn länger als ein gesetzter Timeout, wird das Platoon verlassen, andernfalls der Timer neu gestartet.
					\subparagraph{send\_fv\_hearbeat} Sendet Heartbeats für das FV und startet den Timer.
					\subparagraph{send\_lv\_hearbeat} Sendet Heartbeats für das LV und startet den Timer.
					\subparagraph{get\_vehicle\_id\_param} Bestimmt eine eindeutige ID des Fahrzeugs.
					\subparagraph{set\_role} Bestimmt die Rolle des Fahrzeugs im Platoon, LV oder FV.
				\paragraph{Input}
				    \begin{itemize}
    					\item platooningToggle von Nodelet MessageDistribution
...
\begin{itemize}
	...
	\item Aufsetzen und Bereitstellen Referenz Virtual Machines ROS
...
\end{itemize}
\subsection*{Franz Eichberg}
\begin{itemize}
	\item Kommunikation mit Assystem als Lieferant (Fahrzeugbau, Handling zu erbringende Leistungen von Assystem)
	\item Schulung Fahrzeugbau an Fahrzeug 1
	\item Handling Teileliste
	\item Fahrzeugbau Fahrzeug 2 + 3
	\item Dokumentation Fahrzeugbau, Teileliste
	\item Präsentation Hardware
\end{itemize}
\newpage
\printbibliography
...
\section{Hardware}
\label{appendix:fahrzeugbau}
		\includepdf[pages=1,pagecommand=\subsection{Teileliste Kabel}, offset=0 -2cm]{fahrzeugbau/teilelistekabel-neu.pdf}
		\includepdf[pages=2-]{fahrzeugbau/teilelistekabel-neu.pdf}

		\includepdf[pages=1,pagecommand=\subsection{Teileliste Schrauben}, offset=0 -1.5cm]{fahrzeugbau/teilelisteschrauben-neu.pdf}
		\includepdf[pages=2-]{fahrzeugbau/teilelisteschrauben-neu.pdf}

		\includepdf[pages=1,pagecommand=\subsection{Stoßfänger und Ultraschallsensor}, offset=0 -3cm]{fahrzeugbau/stossfaenger-neu.pdf}
		\includepdf[pages=2-]{fahrzeugbau/stossfaenger-neu.pdf}
		
		\includepdf[pages=1,pagecommand=\subsection{Vordere Halterung und Kamerahalterung}, offset=0 -3cm]{fahrzeugbau/kamerahalterung-neu.pdf}
		
		\includepdf[pages=1,pagecommand=\subsection{Hintere Halterung}, offset=0 -3cm]{fahrzeugbau/hinterehalterung-neu.pdf}
		
		\thispagestyle{empty} % pdf ueberlagerten die Seitenzahlen, Dirty Loesung
		\includepdf[pages=1,pagecommand=\subsection{Montage Metall-Board}, offset=0 -1.2cm, scale=0.9]{fahrzeugbau/metallboard-neu.pdf}
		\includepdf[pages=2-]{fahrzeugbau/metallboard-neu.pdf}

		\includepdf[pages=1,pagecommand=\subsection{Sensormontage Hinterräder}, offset=0 -1.5cm, scale=0.9]{fahrzeugbau/radsensor-neu.pdf}
		\includepdf[pages=2-]{fahrzeugbau/radsensor-neu.pdf}

		\includepdf[pages=1,pagecommand=\subsection{Verkabelung}, offset=0 -3cm]{fahrzeugbau/verkabelung-neu.pdf}
		\includepdf[pages=2-]{fahrzeugbau/verkabelung-neu.pdf}
